...
void executePart1()
{
    ...
    int ca = 0;
    printf("Fixed 32KB Cache size, Select 1 for 16 bytes cache line size, 2 for 32 bytes cache line size, 3 for 128 bytes cache line size : \n");
    scanf("%d", &ca);
    int selection = 0;
    ...
    printf("--------- EXECUTION COMPLETED!!! ---------\n");
        ...
        printf("Select 1 to continue , press any other key to exit: ");
...
void executePart2()
{
 ...
 long cacheSize = 16 * 1024;
    int totalNumberOfBlocks = 0;
    int ca = 0;
    printf("For execution over 64Bytes Cache line size, Select 1 for 16KB Cachesize, 2 for 32KB Cachesize, 3 for 64KB Cachesize \n  \n");
    scanf("%d", &ca);
    int selection;
    printf("Select 1 for fully associative execution, 2 for 2 way execution, 4 for 4 way execution, 8 for 8 way execution: ");
    scanf("%d", &selection);
        ...
        case 1:
            ...
            printf("\n\n******* EXECUTING 16kb cache size and 64 bytes cache line size OR block size *********\n");
            switch (selection)
            {
            case 8:
            ...
            break;
        case 2:
            printf("\n\n******* STARTING 32kb cache size and 64 bytes cache line size/block size*******\n");
            cacheSize = 32 * 1024;
            totalNumberOfBlocks = (int)cacheSize / 64;
            switch (selection)
            {
            ...
            break;
        case 3:
            printf("\n\n********* EXECUTING 64kb cache size and 64 bytes cache line size/block size *********\n");
            cacheSize = 64 * 1024;
            totalNumberOfBlocks = (int)cacheSize / 64;
            switch (selection)
            {
...
int main(int argc, char *argv[])
{
    ...
    int switchCase = 0;
    printf("Select 1 to  execute Fixed cache size 1 or 2 to Fixed cache line size 2 :");
    scanf("%d", &switchCase);
